...
// Fetch announcements
try {
    $announcements = $pdo->query("SELECT title, content, created_at FROM announcements ORDER BY created_at DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
} catch(PDOException $e) {
    $announcements = [];
}
...
// Fetch events
try {
    $events = $pdo->query("SELECT title, description, event_date, location FROM events WHERE event_date >= CURDATE() ORDER BY event_date ASC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
} catch(PDOException $e) {
    $events = [];
}
?>
...
<head>
  ...
  <title>Student Dashboard - CPSU Portal</title>
  ...
  <main class="main-content">
    <header class="header">
      <div>
        ...
        <p class="header-subtitle">Stay updated with the latest announcements and events.</p>
      </div>
    </header>
